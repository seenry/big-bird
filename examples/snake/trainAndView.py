...
for generation in range(max_generation):
    ...
    delay = .075    # Time that pygame waits before starting next game loop
        ...
        while not snake.dead:
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    # Toggle pygame display with space bar
                    if event.key == pygame.K_SPACE:
                        displaying = not displaying
                    # Toggle showcasing with "f" key
                    if event.key == pygame.K_f:
                        showcasing = not showcasing
